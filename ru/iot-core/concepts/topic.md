...
Означает подстановку неограниченного количества топиков: стандартных или дополненных произвольными сабтопиками. Всегда является последним символом в фильтре.

Примеры использования символа **#**:

- `#` — подписаться на все существующие алиасы топиков.
- `$devices/#` — подписаться на все существующие топики от всех устройств.
- `$devices/<ID устройства>/#` — подписаться на все существующие топики от определенного устройства.
- `$devices/<ID устройства>/events/#` — подписаться на все топики с телеметрическими данными от определенного устройства.

#### Символ + {#plus}

Означает подстановку одной части сабтопика, указанной между символами `/`.

Примеры использования символа **+**:

- `$devices/<ID устройства>/+` — подписаться на все топики с телеметрическими данными от определенного устройства и с командами для этого устройства.
- `$devices/<ID устройства>/events/+` — подписаться на все топики с телеметрическими данными от определенного устройства во всех помещениях. Предполагается, что в примере символ `+` замещает помещение.
- `$devices/+/events/bedroom/temperature` — подписаться на все топики от всех устройств с телеметрическими данными о температуре в спальне. 
...
> - Подписка на фильтр `$registries/#` с сертификатом устройства эквивалентна подпискам: `$devices/<ID устройства>/commands`, `$devices/<ID устройства>/commands/#`.
